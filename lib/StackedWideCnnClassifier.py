...
class StackedWideCnnClassifier:
    ...
    def build_network(self,  vocab_size, max_sequence_length, num_outputs, kernel_shape, pool_size=2, embed_dim=50, num_channels=1, ngram_widths=[2,3,5], end_filters=5, use_bias=1, embedding_matrix=None, train_embedding=False, cnn_padding='valid', dropout=0.3):
        ...
        # This will then feed into a stacked series of convolutional layers and finally
        # into a softmax dense layer for output mapping.
        ...
        merged = keras.layers.merge.concatenate([pool1, pool2, pool3])

        stack_cnn1 = keras.layers.Conv1D(end_filters,
                                    input_shape=(max_sequence_length, embed_dim, num_channels),
                                    kernel_size=kernel_shape,
                                    padding=cnn_padding,
                                    activation='relu',
        ...
        stack_drop1 = keras.layers.Dropout(dropout)(stack_cnn1)
        stack_pool1 = keras.layers.MaxPool1D(pool_size=pool_size, strides=None, padding=cnn_padding, data_format='channels_last')(stack_drop1)

        stack_cnn2 = keras.layers.Conv1D(end_filters,
                                         input_shape=(max_sequence_length, embed_dim, num_channels),
                                         kernel_size=kernel_shape,
                                         padding=cnn_padding,
                                         activation='relu',
                                         use_bias=use_bias,
                                         data_format="channels_last",
                                         kernel_initializer=keras.initializers.he_normal(seed=None))(stack_pool1)
        stack_drop2 = keras.layers.Dropout(dropout)(stack_cnn2)
        stack_pool2 = keras.layers.MaxPool1D(pool_size=pool_size, strides=None, padding=cnn_padding, data_format='channels_last')(stack_drop2)

        stack_cnn3 = keras.layers.Conv1D(end_filters,
                                         input_shape=(max_sequence_length, embed_dim, num_channels),
                                         kernel_size=kernel_shape,
                                         padding=cnn_padding,
                                         activation='relu',
                                         use_bias=use_bias,
                                         data_format="channels_last",
                                         kernel_initializer=keras.initializers.he_normal(seed=None))(stack_pool2)
        stack_drop3 = keras.layers.Dropout(dropout)(stack_cnn3)
        stack_pool3 = keras.layers.MaxPool1D(pool_size=pool_size, strides=None, padding=cnn_padding, data_format='channels_last')(stack_drop3)

        stack_flat3 = keras.layers.Flatten()(stack_pool3)


        # Add the softmax layer
        softmaxOut = keras.layers.Dense(num_outputs,
                                        activation='softmax',
                                        kernel_initializer=keras.initializers.he_normal(seed=None))(stack_flat3)
